...
THIRD_PARTY_SOURCES := $(wildcard $(SRC_DIR)/third-party/flatbuffers/*.cpp $(SRC_DIR)/third-party/imgui_md/*.cpp)
THIRD_PARTY_C_SOURCES := $(wildcard $(SRC_DIR)/third-party/md4c/*.c)
...
$(ODIR)/%.inc: $(SRC_DIR)/%.cpp $(SRC_DIR)/%.c
	$(CXX) $(CPPFLAGS) $(DEFINES) $(CXXFLAGS) -H -E $< -o /dev/null 2> $@

.PHONY: includes
includes: $(OBJS:.o=.inc) $(C_OBJS:.o=.inc)
