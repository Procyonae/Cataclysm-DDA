...
namespace
{
generic_factory<flag_ter_furn> ter_furn_flags_all( "flag_ter_furn" );
} // namespace

/** @relates string_id */
template<>
bool ter_furn_flag_id::is_valid() const
{
    return ter_furn_flags_all.is_valid( *this );
}

/** @relates string_id */
template<>
const flag_ter_furn &ter_furn_flag_id::obj() const
{
    return ter_furn_flags_all.obj( *this );
}

flag_ter_furn::operator bool() const
{
    return id.is_valid();
}

const flag_ter_furn &flag_ter_furn::get( const std::string &id )
{
    static const flag_ter_furn null_value = flag_ter_furn();
    const ter_furn_flag_id tf_f_id( id );
    return tf_f_id.is_valid() ? *tf_f_id : null_value;
}

void flag_ter_furn::load( const JsonObject &/*jo*/, const std::string_view )
{
}

void flag_ter_furn::check_consistency()
{
    ter_furn_flags_all.check();
}

void flag_ter_furn::reset()
{
    ter_furn_flags_all.reset();
}

void flag_ter_furn::load_all( const JsonObject &jo, const std::string &src )
{
    ter_furn_flags_all.load( jo, src );
}

void flag_ter_furn::check() const
{
}

void flag_ter_furn::finalize_all()
{
    ter_furn_flags_all.finalize();
}

bool flag_ter_furn::is_ready()
{
    return !ter_furn_flags_all.empty();
}

const std::vector<flag_ter_furn> &flag_ter_furn::get_all()
{
    return ter_furn_flags_all.get_all();
}
