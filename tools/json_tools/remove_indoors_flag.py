...
Removes INDOORS flag from terrain and furniture

Example usage:
py remove_indoors_flag.py ../../data/mods/my_mod

In the unlikely case that INDOORS was the only flag and was flags was the last field in the terrain object there will be a trailing comma that will error the linter

Linter location might want changing below, a regular download of the game release (not the repo) will include a compiled linter
...
def remove_indoors_flag(jo, changed):
    assert "type" in jo
    change = False
    if jo["type"] == "terrain" or jo["type"] == "furniture" :
        if "flags" in jo and "INDOORS" in jo["flags"]:
            jo["flags"].remove("INDOORS")
            if len(jo["flags"]) == 0:
                jo.pop("flags")
            change = True
        if change:
...
for root, directories, filenames in os.walk(args_dict["dir"]):
    for filename in filenames:
        ...
        if path.endswith(".json"):
            readWriteJson(path, remove_indoors_flag)
