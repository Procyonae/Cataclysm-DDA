...
Removes object field from mapgen

Example usage:
py remove_object.py ../../data/mods/my_mod
...
def remove_object(jo, changed):
    assert "type" in jo
    change = False
    if jo["type"] == "mapgen":
        if "object" in jo:
            for key, value in jo["object"].items():
                jo[key] = value;
            jo.pop("object")
            change = True
        if change:
...
for root, directories, filenames in os.walk(args_dict["dir"]):
    for filename in filenames:
        ...
        if path.endswith(".json"):
            readWriteJson(path, remove_object)
